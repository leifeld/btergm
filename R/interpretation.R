...
edgeprob <- function(object, parallel = c("no", "multicore", "snow"), 
  ...
  for (i in 1:length(env$networks)) {
    imat <- matrix(rep(1:nrow(as.matrix(env$networks[[i]])), 
        ncol(as.matrix(env$networks[[i]]))), 
        nrow = nrow(as.matrix(env$networks[[i]])))
    jmat <- matrix(rep(1:ncol(as.matrix(env$networks[[i]])), 
        nrow(as.matrix(env$networks[[i]]))), 
        nrow = nrow(as.matrix(env$networks[[i]])), byrow = TRUE)
  ...
  if (parallel[1] == "snow") {
    ...
    a <- proc.time()
    prob <- mclapply(1:nrow(subs), interpret.parallel, subs, env, coefs, mc.cores = ncpus)
    prob <- unlist(prob)
    b <- proc.time()
    if (verbose == TRUE) {
      message(paste("Time elapsed:"))
    ...
    if (verbose == TRUE) {
      message(paste0("Parallelization is switched off. Computing probabilities sequentially."))
    }
    a <- proc.time()
