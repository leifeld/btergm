...
handleMissings <- function(mat, na = NA, method = "remove", logical = FALSE) {
  ...
  for (i in 1:length(mat)) {
    if (class(mat[[i]]) == "matrix") {
      ...
      onemode[[i]] <- xergm.common::is.mat.onemode(mat[[i]])  # helper function
      directed[[i]] <- xergm.common::is.mat.directed(mat[[i]])  # helper f.
      type[[i]] <- "matrix"
...
adjust <- function(source, target, remove = TRUE, add = TRUE, value = NA, 
  ...
  for (i in 1:length(sources)) {
    ...
    if (class(sources[[i]]) == "network") {
      ...
      sources.onemode[[i]] <- xergm.common::is.mat.onemode(sources[[i]])
      sources.directed[[i]] <- xergm.common::is.mat.directed(sources[[i]])
    } else {
    ...
    if (class(targets[[i]]) == "network") {
      ...
      targets.onemode[[i]] <- xergm.common::is.mat.onemode(targets[[i]])
      targets.directed[[i]] <- xergm.common::is.mat.directed(targets[[i]])
    } else {
